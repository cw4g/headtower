 ...
 * row, a card corner, or a detail header. Each menu item opens a dialog wired
 * straight to a Server Action: the dialog closes once the control plane
 * confirms the change, and a typed reason surfaces inline when it doesn't.
 *
 * Each dialog's working state lives in an inner form that only mounts while
 * the dialog is open, so it always initialises from the node's current values.
 */
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit}>
        <DialogBody>
          <Field label="Display name" htmlFor="menu-rename-input">
            <Input
      ...
      <form onSubmit={handleSubmit}>
        <DialogBody>
          <Field
            ...
            htmlFor="menu-delete-confirm"
          >
            <Input
              id="menu-delete-confirm"
              value={confirm}
              onChange={(e) => {
